...
{Compute the automorphism group of G; CharacteristicSubgroups is a sequence
 of known characteristic subgroups of G; StartGroup defaults to appropriate GL, but could be a subgroup of GL known to contain auts of G which lift}
...
if StartGroup cmpeq [] then  
   ...
   B := StartGroup;
   order := LMGOrder (B);
  //  D1, D2, Dphi := LMGUnipotentRadical (B);
   // Pgp := [D.i : i in [1..Ngens (D)]];
   Pgp := [];
   B := [B.i : i in [1..Ngens (B)]];
end if;
