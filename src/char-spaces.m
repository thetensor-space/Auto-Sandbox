...
intrinsic MyCharSpaces (G::GrpPC: Known := []) -> []
{G p-group; return char subgroups and spaces}
   ...
   for j in [1..#C] do 
      level := C[j][2];
      gens := [G.i : i in [r[level]+1..r[level + 1]]];
      rest := [G.j: j in [r[level + 1] + 1..NPCgens (G)]];
      B := [];
      basis := Basis (C[j][1]); 
      if #basis eq 0 then continue; end if;
      Append (~Spaces, C[j]);
      for b in Basis (C[j][1]) do 
         e := Eltseq (b);
         e := [Z!x : x in e];
         g := &*[gens[i]^e[i]: i in [1..#e]]; 
         Append (~B, g);
      end for;
      S := sub <G | B, rest>;
      Append (~U, S);
